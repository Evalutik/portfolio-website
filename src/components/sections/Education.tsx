...
export function Education() {
    ...
    const currentState = useMemo(() => {
        ...
        const numMarkers = MARKER_Y_POSITIONS.length
        const FIRST_SEGMENT_END = 0.15

        // First segment: draw line to first marker, camera stays at top
        if (scrollProgress < FIRST_SEGMENT_END) {
            const segmentProgress = scrollProgress / FIRST_SEGMENT_END
            const drawProgress = markerPathProgress[0] * segmentProgress
            ...
            return {
                ...
                cardOpacity: triggered ? 1 : 0,
                cardVisible: triggered,
            }
        }

        // Subsequent segments: handle markers 1-4 (indices 0-4 after first)
        ...
        // Calculate draw progress based on phase
        const getDrawProgress = (): number => {
            if (isLastMarker) return currentMarkerPathProg

            const pathToNext = nextMarkerPathProg - currentMarkerPathProg
            if (segmentProgress < 0.55) {
                return currentMarkerPathProg + (pathToNext * 0.6 * (segmentProgress / 0.55))
            } else if (segmentProgress < 0.70) {
                const fadePhase = (segmentProgress - 0.55) / 0.15
                return currentMarkerPathProg + (pathToNext * 0.6) + (pathToNext * 0.1 * fadePhase)
            } else {
                const scrollPhase = (segmentProgress - 0.70) / 0.30
                return currentMarkerPathProg + (pathToNext * 0.7) + (pathToNext * 0.3 * scrollPhase)
            }
        }

        // Calculate viewport offset based on phase
        const getViewportOffset = (): number => {
            const baseOffset = (VIEWPORT_HEIGHT / 2) - currentMarkerY
            if (segmentProgress < 0.70 || isLastMarker) {
                return baseOffset
            }
            const scrollPhase = (segmentProgress - 0.70) / 0.30
            return baseOffset - ((nextMarkerY - currentMarkerY) * scrollPhase)
        }

        // Calculate card opacity based on phase
        const getCardOpacity = (): number => {
            if (segmentProgress < 0.55) return 1
            if (segmentProgress < 0.70) return 1 - ((segmentProgress - 0.55) / 0.15)
            return 0
        }
        ...
        return {
            drawProgress: Math.min(1, Math.max(0, getDrawProgress())),
